...
#if DOLPHIN_BUILD
	MainLines.push_back(new Comment("Honeypot Build Netplay Code Menu", &MENU_TITLE_CHECK_LOCATION));
#else
	MainLines.push_back(new Comment("Honeypot Build Code Menu", &MENU_TITLE_CHECK_LOCATION));
#endif
...
#if DOLPHIN_BUILD
	MainLines.push_back(new Selection("Stagelist", { "ILPM", "ILPM Dubs", "Middle 3", "PMBR" }, 0, STAGELIST_INDEX));
#else
...
void ControlCodeMenu()
{
	...
	If(Reg1, NOT_EQUAL_I_L, 0x7420); //t space
	{
