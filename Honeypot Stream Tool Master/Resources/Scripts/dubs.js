...
const p1NScoreSize = '70px';
const p2NScoreSize = '70px';
...
async function getData(scInfo) {
	...
	let s1int = parseInt(p1NScore);
	let s2int = parseInt(p2NScore);
	...
	if (startup) {
		...
		if (allowIntro) {
			...
			if (s1int + s2int == 0) { //if this is the first game, introduce players
				...
				p1IntroEL.style.fontSize = '100px'; //resize the font to its max size
				resizeText(p1IntroEL); //resize the text if its too large
				p2IntroEL.style.fontSize = '100px';
		...
		updatePlayerName('p1Wrapper', 'p1Name', p1Name);
		...
		updatePlayerName('p2Wrapper', 'p2Name', p2Name);
...
function updateColor(colorID, textID, pColor) {
	...
	// document.getElementById(textID).style['filter'] = 'drop-shadow(0 0 10px ' + getHexColor(pColor) + ')';
	gsap.to(colorEL, {backgroundColor: getHexColor(pColor), duration: fadeInTime});
...
//player text change
function updatePlayerName(wrapperID, nameID, pName) {
	const nameEL = document.getElementById(nameID);
	nameEL.style.fontSize = '60px'; //set original text size
