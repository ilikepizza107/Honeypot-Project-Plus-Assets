...
const roundSize = '36px';
const casterSize = '24px';
const twitterSize = '14px';
...
//variables for sponsor slideshow constant change
let sponsors = document.getElementsByClassName("sponsors");
let sponsorIndex = 0;
const sponsorInterval = 7000;
...
async function getData(scInfo) {
	...
	let p2Score = scInfo['p2NScore'];
	let p2Color = scInfo['p2Color'];
	let p2Character = scInfo['p2Character'];
	let p2Skin = scInfo['p2Skin'];
	let p2WL = scInfo['p2WL'];

	//converting to int for intro to work
	let s1int = parseInt(p1Score);
	let s2int = parseInt(p2Score);
	...
	if (startup) {
		...
		if (allowIntro) {
			...
			if (!s1int) { s1int = 0 };
			if (!s2int) { s2int = 0 };
			if (s1int + s2int == 0) { //if this is the first game, introduce players
				...
				p1IntroEL.style.textShadow = '0px 0px 0px ' + getHexColor(p1Color);
				p2IntroEL.style.textShadow = '0px 0px 0px ' + getHexColor(p2Color);
				...
				if ((s1int + s2int) != 4) { //if its not the last game of a bo5

					//just show the game count in the intro
					midTextEL.textContent = "Game " + (s1int + s2int + 1);
		...
		// if (!caster1 && !caster2 && !twitter1 && !twitter2 && !twitch1 && !twitch2) {
		// 	fadeIn("#ESSLogo");
		// }
		...
		//sponsor stuff
		fadeIn('#NexusLogo');
		//change sponsor at a set interval
		setInterval(() => {
			sponsorChange();
		}, sponsorInterval);
	...
	//now things that will happen constantly
		...
		if (document.getElementById('caster1N').textContent != caster1){
			// if (document.getElementById('ESSLogo').style.opacity == 1) {
			// 	fadeOut("#ESSLogo", () => {
			// 	});
			// }
...
function sponsorChange() {
	fadeOut(sponsors[sponsorIndex], () => {
		sponsorIndex++;
		if(sponsorIndex == sponsors.length){
			sponsorIndex = 0;
		}
		fadeIn(sponsors[sponsorIndex]);
	});
}
...
function updateScore(pNum, pScore, pColor) {
	...
	const name = 'p' + pNum + 'NScore';
	const nScoreEL = document.getElementById(name);
	nScoreEL.textContent = pScore;
	// fadeOut('#' + name, () => {
	// 	nScoreEL.textContent = pScore;
	// 	fadeIn('#' + name);
	// });
...
function sponsFadeIn(itemID) {
	gsap.to(itemID, {delay: .2, opacity: 1, duration: fadeInTime});
}
function sponsFadeOut(itemID, funct) {
...
function updateWL(pWL, playerNum) {
	...
	if (pWL == "W") {
		// pWLEL.setAttribute('src', 'Resources/Overlay/[W2].png')
		pWLEL.textContent= "WINNERS";
		pWLEL.style.left = "56px";
	} else if (pWL == "L") {
		// pWLEL.setAttribute('src', 'Resources/Overlay/[L2].png')
		pWLEL.textContent = "LOSERS";
		pWLEL.style.left = "64px";
	}
...
async function updateChar(pCharacter, pSkin, charID, sagaID) {
	...
	charEL.setAttribute('src', 'Resources/Characters/Cutted/' + pCharacter + '/' + pSkin + '.png');
	...
	if (startup) {charEL.addEventListener("error", () => {
		if (charEL == document.getElementById("p1Character")) {
			charEL.setAttribute('src', 'Resources/Characters/Cutted/Random.png');
		} else {
			charEL.setAttribute('src', 'Resources/Characters/Cutted/Random.png');
		}
	})}
